...
DROP TABLE IF EXISTS PopulationVaccinatedInPercent
CREATE TABLE PopulationVaccinatedInPercent
(
...
#OfPeopleVaccinated numeric,
)

INSERT INTO PopulationVaccinatedInPercent
SELECT dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations
, SUM(CONVERT(INT,vac.new_vaccinations)) OVER (Partition BY dea.Location ORDER BY dea.location,
dea.date) AS #OfPeopleVaccinated
FROM ['covid deaths$'] dea
JOIN PortfolioProject..['covid vaccintiaons$'] vac
ON dea.location = vac.location
AND dea.date = vac.date
...
CREATE VIEW PopulationVaccinatedInPercent AS
SELECT dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations
, SUM(CONVERT(INT,vac.new_vaccinations)) OVER (Partition BY dea.Location ORDER BY dea.location,
dea.date) AS #OfPeopleVaccinated
FROM ['covid deaths$'] dea
